...
# Koordinat episenter Gempa Cianjur 2022
eq_lat = -6.853
eq_lon = 107.095
...
# Hitung jarak episentral
for sta, info in stations.items():

    sta_lat = info["coords"][0]
    sta_lon = info["coords"][1]
    elevasi = info["coords"][2]
    ...
    results.append({
        ...
        "Longitude": sta_lon,
        "Elevasi (m)": elevasi,
        "Jarak Episenter (km)": round(dist_km, 2)
    })
...
df = df.sort_values(by="Jarak Episenter (km)")
...
df.to_csv("jarak_stasiun_cianjur.csv", index=False)
